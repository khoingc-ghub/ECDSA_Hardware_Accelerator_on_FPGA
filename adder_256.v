module gps_gpsc #(
    parameter W = 16 // K ch th??c kh?i 16 bit
) (
...
module adder_256 (
    ...
    input  [255:0] P,      // Modun P
    input          SUB,    // Tin hieu chon cong/tru
    output [255:0] S       // Tong hoac hieu 256-bit
);
    wire [15:0] fg, fp;    // Generate vs propagate tu GPS/GPSc
    wire [15:0] sg, sp;    // Generate vs propagate tu GPSc
    wire [16:1] fpc, spc;  // Carry tu PPN cho GPS v  GPSc
    wire [15:0] fs [15:0]; // Tong cac b? tu GPS/GPSc
    wire [15:0] ss [15:0]; // Tong cac b? tu GPSc
    wire [255:0] B_selected; // B hoac ~B
    wire [255:0] IP_selected; // 2^n - P hoac P
    wire sel;              // Tin hieu chon mux

    // Mux cho B: chon B hoac ~B dua tren SUB
    assign B_selected = SUB ? ~B : B;

    // Mux cho IP: chon 2^n - P (t?c ~P + 1) hoac P dua tren SUB
    assign IP_selected = SUB ? P : (~P + 1);

    // Khoi tao 16 module (GPS toi i=0, GPS toi i=1 ??n 15)
    genvar i;
    generate
        // Module GPS t?i i=0, cin = 0
        gps_gpsc #(
            .W(16)
        ) gps_inst_0 (
            .a(A[15:0]),
            .b(B_selected[15:0]),
            .cin(1'b0), // cin = 0 cho GPS
            .s(fs[0]),
            .g(fg[0]),
            .p(fp[0])
        );

        // Module GPS cho c c kh?i i=1 ??n 15
        ...
        // Module GPSc cho t?t c? c c kh?i i=0 ??n 15
        for (i = 0; i < 16; i = i + 1) begin: gpsc_loop
            gps_gpsc #(
                .W(16)
            ) gpsc_inst (
                .a(fs[i]), // T?ng t? GPS
                .b(IP_selected[16*i+15:16*i]), // To n h?ng th? hai l  2^n - P ho?c P
                .cin(i == 0 ? 1'b0 : fg[i-1]), // cin = 0 (i=0) ho?c fg[i-1]
                .s(ss[i]),
                .g(sg[i]),
                .p(sp[i])
            );
    ...
    // Logic ch?n sel d?a tr n Figure 6 v  Figure 7
    assign sel = SUB ? ~fpc[16] : (fpc[16] | spc[16]);

    // ?i?u ch?nh t?ng c?c b? b?ng carry cho c? SUB = 0 v  SUB = 1
    wire [15:0] final_sum [15:0];
    generate
        for (i = 0; i < 16; i = i + 1) begin: mux_loop
            assign final_sum[i] = sel ? (ss[i] + spc[i+1]) : (fs[i] + fpc[i+1]); // C?ng carry d?a tr n sel
        end
    endgenerate

    // G?p c c t?ng c?c b? th nh t?ng/hi?u 256-bit
